...
namespace FundooApp.API.Controllers
{
    ...
    public class NoteController : ControllerBase
    {
        ...
        {
            var notes = await _noteBusiness.GetNotesByLabelAsync(GetUserId(), labelId);
            return Ok(new ResponseDTO<IEnumerable<NoteResponseDTO>> { Success = true, Message = "Notes filtered by label.", Data = notes });
        }
        ...
        public async Task<IActionResult> AddLabelToNote(int noteId, int labelId)
        {
            try
            {
                var note = await _noteBusiness.AddLabelToNoteAsync(noteId, labelId, GetUserId());
                return Ok(new ResponseDTO<NoteResponseDTO> { Success = true, Message = "Label added to note.", Data = note });
            }
            catch (NoteNotFoundException ex)
            {
                return NotFound(new ResponseDTO<string> { Success = false, Message = ex.Message });
        ...
        public async Task<IActionResult> RemoveLabelFromNote(int noteId, int labelId)
        {
            try
            {
                var note = await _noteBusiness.RemoveLabelFromNoteAsync(noteId, labelId, GetUserId());
                return Ok(new ResponseDTO<NoteResponseDTO> { Success = true, Message = "Label removed from note.", Data = note });
            }
            catch (NoteNotFoundException ex)
            {
                return NotFound(new ResponseDTO<string> { Success = false, Message = ex.Message });
